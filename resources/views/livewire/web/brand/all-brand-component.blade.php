                            ...
                            <div class="row row-cols-1 row-cols-lg-4">
                                ...
                                <div class="col">
                                    <div class="card rounded-0 product-card">
                                        <a href="{{ route('web.brand_products',['key'=>$value->id]) }}">

                                            @if($value->image)
                                                <img src="{{ asset('') }}uploads/brands/{{ $value->image }}" class="card-img-top border-bottom bg-dark-1" alt="...">
                                            @else
                                                <img src="{{ asset('') }}uploads/logo/default_logo.png" class="card-img-top border-bottom bg-dark-1" alt="...">
                                            @endif
                                        </a>
                                        <div class="list-group list-group-flush">
                                            <a href="{{ route('web.brand_products',['key'=>$value->id]) }}" class="list-group-item bg-transparent">
                                                <h6 class="mb-0 text-uppercase">{{ $value->name }}</h6>
                                            </a>    
                                            <a href="{{ route('web.brand_products',['key'=>$value->id]) }}" class="list-group-item bg-transparent d-flex justify-content-between align-items-center">
                                            Total Products
